...
ggplot(booking_price, aes(comfort_score, max_price)) +
...
Tras los resultados que se han obtenido observamos que hay distintas variables que pueden tener una mayor influencia sobre nuestra variable objetivo. Por un lado, si nos basamos en lo obtenido tras el análisis de correlación hayamos que variables como **kitchen**, **is_apartment**, **elevator**, **private_bathroom**, **num_rooms**, **value_for_money_score**, **heating**, **pet_friendly** entre otros tienen una gran relevancia. Por otro lado, variables como **air_conditioning** o **free_wifi** han mostrado una relevancia significativa durante el contraste de hipótesis. 

En este apartado comenzaremos tratando de ajustar una regresión lineal simple aproximando la variable dependiente **page_count** con la variable explicativa que ha proporcionado una mayor correlación, que ha resultado ser **kitchen**.

```{r}
#Create the linear model
mrl <- lm(page_count~kitchen, data=booking)

#Evaluate the data
summary(mrl)
```

Obtenemos un valor de R-squared de 0.0127, lo que es un valor muy bajo y representa una aproximación muy mala de la variable dependiente.
...
Observamos que habiendo empleado tan solo 4 variables, de las 8 empleadas previamente, el R-squared tan solo se ha visto reducido en 0.0005.
...
```{r}
#Create the linear model
mrl_complete <- lm(page_count~staff_score + value_for_money_score + location_score + free_wifi_score + min_price + balcony + pet_friendly + air_conditioning + private_bathroom + free_wifi + swimming_pool + non_smoking_rooms + mean_flights + length_description + safe + kitchen, data = booking)

#Evaluate the data
summary(mrl_complete)
```

Con todas las variables la R-squared ha aumentado hasta 0.055, que sigue siendo un resultado muy bajo, pero dificil de mejorar con unos datos tan poco correlacionados con la variable objetivo.

De todas las variables empleadas se observa que las que tienen una mayor influencia son: **staff_score**, **value_for_money_score**, **free_wifi_score**, **balcony**, **pet_friendly**, **free_wifi**, **air_conditioning**, **swimming_pool**, **mean_flights** y **length_description** y **kitchen**. A continuación se aproxima la variable dependiente basándonos exclusivamente en estas variables.

```{r}
#Create the linear model
mrl_final <- lm(page_count~staff_score + value_for_money_score + free_wifi_score + balcony + pet_friendly + air_conditioning + free_wifi + swimming_pool + mean_flights + length_description + kitchen, data=booking)
...
Anteriormente se ha observado una correlación entre el precio de la habitación y sus características, en concreto existen varias variable con una correlación moderada con el precio máximo de esta. A continuación, se tratará de predecir el precio máximo de la habitación mediante una regresión lineal múltiple empleando el resto de variables para ello:

```{r}
#Create the linear model
mrl_price <- lm(max_price~ postal_code + balcony + pet_friendly + latitude + air_conditioning + private_bathroom + free_wifi + staff_score + swimming_pool + non_smoking_rooms + location_score + mean_flights + length_description + facilities_score + hotel_score + safe + cleanliness_score + comfort_score, data=booking)

#Evaluate the data
summary(mrl_price)
```

Con ello obtenemos una R-squared de 0.2572, con lo que podemos observar que variables que tienen una mayor correlación con la variable objetivo nos proporcionarán unos resultados más apropiados. A partir de los resultados mostrados podemos mejorar la regresión empleando tan solo aquellas variables que han aportado más información en la regresión, es decir, todas excepto **pet_friendly**, **private_bathroom** y **hotel_score**. 

A continuación se emplea una regresión lineal múltiple exceptuando dichas variables.

```{r}
#Create the linear model
mrl_price <- lm(max_price~postal_code + balcony + latitude + air_conditioning + free_wifi + staff_score + swimming_pool + non_smoking_rooms + location_score + mean_flights + length_description + facilities_score + safe + cleanliness_score + comfort_score, data=booking)

#Evaluate the data
summary(mrl_price)
```

Se observa que aun habiendo reducido el número de variables, mantenemos una R-squares ligeramente superior 0.2571. Hay que tener en cuenta que los resultados siguen sin ser satisfactorios, ya que una R-squared de 0.257 es bastante baja, no obstante es mucho mejor que lo obtenido tratando de predecir la variable page_count. Esta mala predicción es producida como se había comentado previamente debido a la baja correlación de las variables con la variable objetivo.
