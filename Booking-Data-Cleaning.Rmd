...
# c(1:4) es por el output de knitr, más adelante lo cambiamos.
head(booking[is.na(booking$postal_code),c(1:4)], 5)
...
head(booking[is.na(booking$hotel_score),c(1:4)], 5)

```

```{r}

tail(booking[booking$hotel_score == "-1",c(1:4)], 5)
...
ggplot(booking, aes(y = latitude))+
  geom_boxplot(color = "red", fill = "navy")
...
booking %>%
  select(-c("hotel_description", "room_data")) %>%
...
* Para comprobar normalidad hacer uso de qqplots y el test kolmogorov-smirvo o el lilliefors.
